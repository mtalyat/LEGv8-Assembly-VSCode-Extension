...
import { LEGv8Runtime, LEGv8Breakpoint } from './LEGv8Runtime';
...
export class LEGv8DebugSession extends LoggingDebugSession {
    // only one thread, so hard code the id
    ...
    public constructor() {
        super("legv8-assembly-debug.txt");

        // 0 indexing for both row and col
        this.setDebuggerLinesStartAt1(false);
        this.setDebuggerColumnsStartAt1(false);

        // create runtime and initialize events
        this._runtime = LEGv8Runtime.getNewInstance(false);

        this._runtime.on("stopOnStep", () => {
            this.sendEvent(new StoppedEvent("step", LEGv8DebugSession.THREAD_ID));
        });
        this._runtime.on("stopOnBreakpoint", () => {
            this.sendEvent(new StoppedEvent("breakpoint", LEGv8DebugSession.THREAD_ID));
        });
        this._runtime.on('stopOnException', () => {
            this.sendEvent(new StoppedEvent('exception', LEGv8DebugSession.THREAD_ID));
        });
    ...
    protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
        ...
        response.body = {
            threads: [
                new Thread(LEGv8DebugSession.THREAD_ID, "thread 1")
            ]
        };
        this.sendResponse(response);
    }

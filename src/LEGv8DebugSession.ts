...
import { LEGv8Runtime, IRuntimeBreakpoint, FileAccessor, RuntimeVariable, timeout, IRuntimeVariableType } from './LEGv8Runtime';
...
export class LEGv8DebugSession extends LoggingDebugSession {
    ...
    public constructor(fileAccessor: FileAccessor) {
        ...
        this._runtime = new LEGv8Runtime(fileAccessor);

        // setup event handlers
        this._runtime.on('stopOnEntry', () => {
            this.sendEvent(new StoppedEvent('entry', LEGv8DebugSession.threadID));
        });
        this._runtime.on('stopOnStep', () => {
            this.sendEvent(new StoppedEvent('step', LEGv8DebugSession.threadID));
        });
        this._runtime.on('stopOnBreakpoint', () => {
            this.sendEvent(new StoppedEvent('breakpoint', LEGv8DebugSession.threadID));
        });
        this._runtime.on('stopOnDataBreakpoint', () => {
            this.sendEvent(new StoppedEvent('data breakpoint', LEGv8DebugSession.threadID));
        });
        this._runtime.on('stopOnInstructionBreakpoint', () => {
            this.sendEvent(new StoppedEvent('instruction breakpoint', LEGv8DebugSession.threadID));
        });
        this._runtime.on('stopOnException', (exception) => {
            if (exception) {
                this.sendEvent(new StoppedEvent(`exception(${exception})`, LEGv8DebugSession.threadID));
            } else {
                this.sendEvent(new StoppedEvent('exception', LEGv8DebugSession.threadID));
            }
        });
    ...
    protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
        ...
        response.body = {
            threads: [
                new Thread(LEGv8DebugSession.threadID, "thread 1"),
                new Thread(LEGv8DebugSession.threadID + 1, "thread 2"),
            ]
        };
        this.sendResponse(response);
    }

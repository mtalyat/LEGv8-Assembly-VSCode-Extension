...
export interface LEGv8Breakpoint {
    id: number;
    line: number;
    verified: boolean;
}
...
export class LEGv8Runtime extends EventEmitter {
    private static _instance: LEGv8Runtime;
    ...
    private _dataFile = '';
    private _fileBytes: Buffer = Buffer.alloc(0);

    private _gettingData = false;
    private _dataTotal = 0;
    private _dataReceived = 0;
    private _dataBytes: Buffer = Buffer.alloc(0);
    ...
    private _breakPoints = new Map<string, LEGv8Breakpoint[]>();
    private _breakPointMap = new Map<string, Map<number, LEGv8Breakpoint>>();
    ...
    public static getNewInstance(isRepl = false): LEGv8Runtime {
        return new LEGv8Runtime(isRepl);
    }

    public static getInstance(reload = false): LEGv8Runtime {
        let cscs = LEGv8Runtime._instance;
        if (cscs === null || reload ||
            (!cscs._connected && !cscs._init) ||
            !Data.sameInstance(cscs._instanceId)) {
            LEGv8Runtime._instance = new LEGv8Runtime(false);
        }
        return LEGv8Runtime._instance;
    }
    ...
    splitREPL(repl: string): [string, Array<string>] {
            ...
            switch (ch) {
                ...
                case '}':
                    if (!inQuotes && !inComments) {
                        levelCurly--;
                        if (levelCurly < 0) {
                            LEGv8Runtime.throwErrorMsg(repl, levelCurly, lineNumberCurly, lineNumber, curlyErrorMsg);
                        }
                ...
                case ']':
                    if (!inQuotes && !inComments) {
                        levelBrackets--;
                        if (levelBrackets < 0) {
                            LEGv8Runtime.throwErrorMsg(repl, levelBrackets, lineNumberBrack, lineNumber, bracketErrorMsg);
                        }
                    }
                    break;
                case '(':
                ...
                case ')':
                    if (!inQuotes && !inComments) {
                        ...
                        if (levelParentheses < 0) {
                            LEGv8Runtime.throwErrorMsg(repl, levelParentheses, lineNumberPar, lineNumber, parenthErrorMsg);
                        }
                    }
                    break;
            }
        ...
        if (error) {
            if (inQuotes) {
                LEGv8Runtime.throwErrorMsg(repl, 1, lineNumberQuote, lineNumber, quoteErrorMsg);
    ...
    public connectToDebugger(): void {
        ...
        if (this._connectType === "sockets") {
            ...
            this._debugger.on('data', (data: string | Buffer | Uint8Array) => {
                ...
                if (this._gettingData) {
                    if (this._dataReceived === 0) {
                        this._dataBytes = data as Buffer;
                        ...
                        this._dataBytes = Buffer.concat([this._dataBytes, data as Uint8Array], totalLength);
    ...
    protected processFromDebugger(data: any) {
        ...
        if (this._gettingFile) {
            ...
            if (this._fileReceived >= this._fileTotal) {
                ...
                fs.writeFileSync(this._dataFile, buffer, (err: Error) => {
                    if (err) {
                        throw err;
                    }
                });
    ...
    disconnectFromDebugger() {
        ...
        this.makeInvalid();
        Data.getNextId();
        LEGv8Runtime._instance = LEGv8Runtime.getInstance(true);
    }
    ...
    public setBreakPoint(path: string, line: number): LEGv8Breakpoint {
        ...
        const bp = <LEGv8Breakpoint>{ verified: false, line, id: this._breakpointId++ };
        let bps = this._breakPoints.get(lower);
        if (!bps) {
            bps = new Array<LEGv8Breakpoint>();
        ...
        if (!bpMap) {
            bpMap = new Map<number, LEGv8Breakpoint>();
        }
        bpMap.set(line, bp);
    ...
    private getBreakPoint(line: number): LEGv8Breakpoint | undefined {
    ...
    public clearBreakPoint(path: string, line: number): LEGv8Breakpoint | undefined {
